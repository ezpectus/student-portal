...
- `JWKS_URI` - Optional JWKS endpoint URL for remote JWT signature verification
- `JWT_ISSUER` - Optional expected issuer for remote JWT verification
...
- CSRF protection: double-submit cookie + Origin validation in middleware; `requireCsrf()` guard in all mutating server actions (`src/lib/csrf.ts`)
...
- Rate limiting on login (10 per 15min) and password reset (5 per 15min)
- Code of honor acceptance required for new users
- External JWT tokens checked for expiration even without signature verification
- SQLite WAL mode enabled to prevent SQLITE_BUSY under concurrent writes
- File uploads: 5MB limit, MIME type allow-list for avatar uploads
- Audit logging: all admin/grade mutations logged; failures non-blocking (logged to console)
...
- `docs/infrastructure/` — 8 files covering stack, architecture, request pipeline, database, auth/security, Docker deployment, frontend components, monitoring
- `docs/theory/` — 5 files covering architecture patterns, React Server Components, security concepts, database concepts, development workflow
- `docs/CLAUDE.md` — this file (quick reference for AI assistants)
